                ...
                <div class="row">
                    <div class="col-sm-12" style="height: 25px;">
                        <div class="ibox-tools">
                            <div class="adm-fa-hover"><a href="javascript:window.location.href='{{route('lbb.store.recharge')}}'"><i class="fa fa-refresh"></i></a></div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
	$("#table_list_2").jqGrid({
        url: "{{route('lbb.store.recharge.list', ['param'=>'type'])}}",
		datatype: "json",
		...
		colModel: [
			...
			{name:"user",index:"user",align: "center",editable:true,width: 50,search: false},
			{name:"category",index:"store_category",align: "center",width: 40,editable:false,search: false},
            {name:"recharge_url",index:"recharge_url",align: "center",editable:false,search: false},
            {name:"recharge_num",index:"recharge_num",align: "center",editable:false,width: 40,search: false},
            ...
            if (!id) {
				...
				layer.confirm('确定要处理该条申请吗？', {btn : [ '确定', '取消' ]}, function(index) {
					...
					$.ajax({
						...
						dataType:'json',
						url:'{{route('lbb.store.recharge.dealwith')}}',
