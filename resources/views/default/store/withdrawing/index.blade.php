...
$(document).ready(function() {
	$("#table_list_2").jqGrid({
		...
		colModel: [
            ...
            {name:"withdraw_url",index:"withdraw_url",align: "center",editable:false,search: false},
            {name:"withdraw_num",index:"withdraw_num",align: "center",editable:false,width: 40,search: false},
		...
		onClickButton:function(){
            ...
            if (!id) {
                ...
                if (rowData.recharge_status == 1) {
					window.top._toastr('该提现申请已处理，不能删除！', 'error', '错误提示');return false;
                }
				...
				layer.confirm('确定要删除该条申请吗？', {btn : [ '确定', '取消' ]}, function(index) {
					...
					$.ajax({
						...
						data:{'withdraw_id':rowData.withdraw_id},
						dataType:'json',
						url:'{{route('lbb.store.withdrawing.del')}}',
						success:function(json){
							layer.close(recharge);
							if (json.code) {
								layer.closeAll();
