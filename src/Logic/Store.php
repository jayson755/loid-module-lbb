...
class Store{
    ...
    public function getStoreByUser(LbbUserModel $user, array $fields = []){
        if ($fields) {
            return StoreModel::where('user_id', $user->lbb_user_id)->select($fields)->orderBy('store_id', 'desc')->get();
        } else {
            return StoreModel::where('user_id', $user->lbb_user_id)->orderBy('store_id', 'desc')->get();
        }
    }
